...
import { parsePath } from './PathParticle';
...
export default class Emitter
{
	...
	/**
	 * binded to by spawnPath when spawnPathData has valid parameters
	 */
	protected _spawnPath: () => void;
	/**
	 * Spawns particles in a given path and then uses one of
	 * the other spawn functions after it repositions
	 * the initial spawnPos.
	 * @property {any} spawnPath
	 */
	public spawnPathData: ({path:string, min_x: number, max_x: number});
	/**
	...
	/**
	 * Indicator if the new spawn position should
	 * be found within a path
	 * @private
	 */
	private spawnInPath: boolean;
	...
	public init(art: any, config: any)
	{
		...
		if(config.spawnPathData) {
			const { path, min_x, max_x } = config.spawnPathData;
			this.spawnInPath = true;
			this._spawnPath = (() => {
				this.spawnPath(path, min_x, max_x);
				this.updateOwnerPos(config.pos.x, config.pos.y);
			})
		}
	 ...
	 */
	protected spawnPath(pathString: string, min_x: number, max_x: number) : void
	{
		let path: Function;
		...
		let randomX = ParticleUtils.getRandomInt(min_x, max_x);
